defmodule CorpusBuilder.TwitterSearch do
  ...
  def search(lang, mood, query_count) do
    max_id = inner_search([q: query(lang, mood), count: @page_size], lang, mood)
    if query_count > 1, do: search(lang, mood, max_id, query_count - 1)
    :ok
  end

  def search(lang, mood, max_id, query_count) do
    max_id = inner_search([q: query(lang, mood), count: @page_size, max_id: max_id], lang, mood)
    if query_count > 1, do: search(lang, mood, max_id, query_count - 1)
    :ok
  end
  ...
  defp inner_search(params, lang, mood) do
    RateLimiter.handle_rate_limit(fn ->
      ...
      Task.start_link(fn -> process_search_output(json, lang, mood) end)
      new_max_id(json)
    end)
  end
